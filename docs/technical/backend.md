...
- `PRONUNCIATION_TRANSCRIPT_WEIGHT`, `PRONUNCIATION_CLEAR_THRESHOLD`, `PRONUNCIATION_PRACTICE_THRESHOLD`, `PRONUNCIATION_MIN_CLARITY_FOR_CLEAR`, `PRONUNCIATION_MIN_SOUND_FOR_CLEAR`, `PRONUNCIATION_MIN_RHYTHM_FOR_CLEAR`, `PRONUNCIATION_MIN_RHYTHM_HARD_FLOOR`, `PRONUNCIATION_COMPONENT_TOLERANCE`: pronunciation scoring controls. Known-target lesson/repeat drills stay audio-first, but small clarity/sound/rhythm dips are treated with tolerance, and rhythm only blocks clear when it makes the sentence hard to follow.
- `VOICE_FORM_FIELD_SIZE_BYTES`: multipart text field limit for voice/pronunciation uploads, default `524288`. The mobile app sends compact recent conversation history for voice turns, and oversized history returns a JSON `413` instead of a raw upload parser error.
